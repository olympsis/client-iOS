...
struct NewTournamentEvent: View {
    ...
    func CreateEvent() async {
        ...
        let participant = Participant(uuid: uuid, status: "yes", createdAt: timestamp)
        ...
        let event = Event(id: nil, type: "tournament", poster: uuid, organizers: organizers, field: FieldDescriptor(type: "internal", id: eventField.id, location: nil), imageURL: selectedImage, title: eventTitle, body: eventBody, sport: eventSport.rawValue, level: eventLevel,startTime: setStartTime,actualStopTime: nil, minParticipants: Int(eventMinParticipants), maxParticipants: Int(eventMaxParticipants), participants: [participant], visibility: "public", data: nil, createdAt: timestamp, externalLink: eventExternalLink)
...
#Preview {
    NewTournamentEvent(eventField: FIELDS[0])

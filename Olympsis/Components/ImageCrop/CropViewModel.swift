...
class CropViewModel: ObservableObject {
    ...
    init(
        ...
        switch self.configuration.maskShape {
        case .circle, .square:
    ...
    func calculateDragGestureMax() -> CGPoint {
        let yLimit = ((imageSizeInView.height / 2) * scale) - maskHeightRadius
        let xLimit = ((imageSizeInView.width / 2) * scale) - maskWidthRadius
    ...
    func cropImage(_ image: UIImage) -> UIImage? {
        ...
        case .square:
            ...
            let cropRect = calculateCropSquare(orientedImage)
        ...
        case .rectangle:
            ...
            let cropRect = calculateCropRectangle(orientedImage)
     ...
     */
    private func calculateCropSquare(_ orientedImage: UIImage) -> CGRect {
        ...
        let cropRadiusInOriginalImage = (maskWidthRadius * factor) / scale
    ...
    /**
     ...
     - Returns: A CGRect representing the rectangle to crop.
     */
    private func calculateCropRectangle(_ orientedImage: UIImage) -> CGRect {
        // The relation factor of the originals image width/height
        // and the width/height of the image displayed in the view (initial)
        let factor = min(
            (orientedImage.size.width / imageSizeInView.width), (orientedImage.size.height / imageSizeInView.height)
        ...
        // Calculate the crop radius inside the original image which based on the mask radius
        let cropWidthInOriginalImage = (maskWidthRadius * factor) / scale
        let cropHeightInOrginalImage = (maskHeightRadius * factor) / scale
        ...
        // Calculates the x coordinate of the crop rectangle inside the original image
        let cropRectX = (centerInOriginalImage.x - cropWidthInOriginalImage) - (offsetX / scale)
        // Calculates the y coordinate of the crop rectangle inside the original image
        let cropRectY = (centerInOriginalImage.y - cropHeightInOrginalImage) - (offsetY / scale)
        ...
        let cropRect = CGRect(
            x: cropRectCoordinate.x,
            y: cropRectCoordinate.y,
            width: cropWidthInOriginalImage * 2,
            height: cropHeightInOrginalImage * 2
        )
